    ...
    <table class="table table-sm table-hover mt-3 itemTable" style="font-size:small">
      <thead class="table-light">
        <tr class="text-center">
          ...
          <th scope="col" style="width:85px">펀드명</th>
          <th scope="col" style="width:85px">펀드유형</th>
          <th scope="col" style="width:85px">공모/사모</th>
          <th scope="col" style="width:85px">단위</th>
          <th scope="col" style="width:85px">모/자</th>
          <th scope="col" style="width:85px">설정일자</th>
          <th scope="col" style="width:90px">예탁원 코드</th>
          <th scope="col" style="width:85px">운용사</th>
          <th scope="col" style="width:85px">수탁사</th>
          <th scope="col" style="width:85px">사무수탁사</th>
      ...
      <tbody class="table-group-divider" >
        <c:forEach var="fund" items="${list}" varStatus="status">
            <tr class="align-middle">
              ...
              <td class="text-center">${fund.fnd01FundNm}</td>
              <td class="text-center">${fund.fnd01FundTypeNm}</td>
              <td class="text-center">${fund.fnd01PublicCdNm}</td>
              <td class="text-center">${fund.fnd01UnitCdNm}</td>
              <td class="text-center">${fund.fnd01ParentCdNm}</td>
              <td class="text-center">${fund.fnd01StartDate}</td>
              <td class="text-center">${fund.fnd01KsdItemCd}</td>
              <td class="text-center">${fund.fnd01MngCoCdNm}</td>
              <td class="text-center">${fund.fnd01TrustCoCdNm}</td>
              <td class="text-center">${fund.fnd01OfficeCoCdNm}</td>
              <td class="text-end">
...
$(document).ready(function () {
   ...
   $('.btnRetrieve').on('click', function(){
    ...
    $('.btnDelete').on("click", function(){
    	...
    	if(confirm(fnd01FundNm + " 기관정보를 삭제하시겠습니까?")){
