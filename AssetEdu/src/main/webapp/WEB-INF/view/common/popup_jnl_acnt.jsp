...
<main class="container-fluid mt-3">
 	<form id="form1" action="/popup/jnl/acnt/parent" method="GET">
 		<input type="hidden" name="parentCdId" value="${param.parentCdId }"/>
 		<input type="hidden" name="parentNmId" value="${param.parentNmId }"/>
 		<input type="hidden" name="slipYn" value="${param.slipYn }"/>
	...
	<table class="table table-hover table-sm reprTable" style="font-size:small">
	  <thead class="table-light">
	    <tr class="text-center">
	      ...
	      <th scope="col" style="width:90px">계정코드</th>
          <th scope="col" style="width:150px">계정명</th>
	    </tr>
	  </thead>
	  <tbody class="table-group-divider" >
	  	<c:forEach var="acnt" items="${list}" varStatus="status"> 
		    <tr class="align-middle">
		      <td><input type="radio" data-acnt-cd="${acnt.jnl10AcntCd }" data-acnt-nm="${acnt.jnl10AcntNm }" id="jnl10AcntCd_${status.count }" name="jnl10AcntCd"/></td>
              <td class="text-center"><label for="jnl10AcntCd_${status.count }">${acnt.jnl10AcntCd }</label></td>
              <td class="text-center"><label for="jnl10AcntCd_${status.count}">${acnt.jnl10AcntNm }</label></td>
		    </tr>
	    </c:forEach>
	  </tbody>
	</table>
